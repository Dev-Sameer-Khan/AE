...
import { ArrowRight, ArrowLeft, CheckCircle2, Globe2, Wrench, Settings } from 'lucide-react';
...
const productCategories = [
  {
    ...
    description: { en: "High-performance diesel engines.", ar: "محركات ديزل عالية الأداء." }
  },
  {
    name: { en: "Compressors", ar: "الضواغط" },
    image: "https://images.unsplash.com/photo-1621905251189-08b45d6a269e?auto=format&fit=crop&q=80&w=800",
    link: "/products?category=Compressors",
    description: { en: "Industrial air compressors.", ar: "ضواغط هواء صناعية." }
  },
  {
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h1 className="text-5xl md:text-7xl font-display font-bold text-white mb-6 leading-tight tracking-tight">
                {language === 'en' ? heroSlides[currentSlide].title : t('hero.title')}
              </h1>
              <p className="text-lg md:text-2xl text-slate-300 mb-10 max-w-2xl mx-auto font-light">
                {language === 'en' ? heroSlides[currentSlide].subtitle : t('hero.subtitle')}
              </p>
          ...
          {heroSlides.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                currentSlide === index ? 'bg-[var(--color-orange)] w-8' : 'bg-white/50 hover:bg-white'
              }`}
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center justify-center w-full py-3 bg-slate-50 text-[var(--color-navy)] font-semibold rounded-lg group-hover:bg-[var(--color-navy)] group-hover:text-white transition-colors duration-300"
                  >
                    View Details
                  </Link>
                </div>
              ...
              <motion.div key={index} variants={itemVariants}>
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2">{category.name[language]}</h3>
                    <p className="text-slate-300 text-sm">{category.description[language]}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
            ...
            >
              ...
              <div className="space-y-4 mb-10">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-[var(--color-navy)]">Global Reach</h4>
                    <p className="text-slate-500 text-sm">Connecting markets across India, Gulf, and Turkey.</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center flex-shrink-0 text-[var(--color-orange)]">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-[var(--color-navy)]">Premium Quality</h4>
                    <p className="text-slate-500 text-sm">Sourcing only genuine and OEM standard parts.</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center flex-shrink-0 text-[var(--color-orange)]">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-[var(--color-navy)]">Expert Maintenance</h4>
                    <p className="text-slate-500 text-sm">Comprehensive support for diesel machines.</p>
                  </div>
                </div>
              </div>
              ...
              <Link
                to="/about"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-navy)] text-white font-semibold rounded-lg hover:bg-[var(--color-navy-light)] transition-colors"
              >
                {t('about.readMore')}
              </Link>
            </motion.div>
